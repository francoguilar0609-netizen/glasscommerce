...
export default function Home() {
  ...
  async function placeOrder(){if(!itemCount||checkingOut)return;setCheckingOut(true);const idempotencyKey=window.sessionStorage.getItem("glasscommerce-checkout-key")||crypto.randomUUID();window.sessionStorage.setItem("glasscommerce-checkout-key",idempotencyKey);const response=await fetch("/api/checkout",{method:"POST",headers:{"Content-Type":"application/json","Idempotency-Key":idempotencyKey},body:JSON.stringify({items:entries.map(i=>({productId:i.id,quantity:i.quantity}))})});const data=await response.json();setCheckingOut(false);if(response.status===401){window.location.assign("/login?return_to=/");return}if(data.checkoutUrl){window.sessionStorage.removeItem("glasscommerce-checkout-key");window.location.assign(data.checkoutUrl);return}setNotice(data.error||"No se pudo iniciar el pago.")}
