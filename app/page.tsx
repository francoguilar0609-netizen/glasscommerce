...
export default function Home() {
  ...
  async function placeOrder(){if(!itemCount||checkingOut)return;setCheckingOut(true);const idempotencyKey=window.sessionStorage.getItem("glasscommerce-checkout-key")||crypto.randomUUID();window.sessionStorage.setItem("glasscommerce-checkout-key",idempotencyKey);const response=await fetch("/api/checkout",{method:"POST",headers:{"Content-Type":"application/json","Idempotency-Key":idempotencyKey},body:JSON.stringify({items:entries.map(i=>({productId:i.id,quantity:i.quantity}))})});const data=await response.json();setCheckingOut(false);if(response.status===401){window.location.assign("/login?return_to=/");return}if(data.checkoutUrl){window.sessionStorage.removeItem("glasscommerce-checkout-key");window.location.assign(data.checkoutUrl);return}if(data.code==="idempotency_conflict"||data.code==="checkout_terminal")window.sessionStorage.removeItem("glasscommerce-checkout-key");setNotice(data.error||"No se pudo iniciar el pago.")}
  ...
  return <main>
    ...
    <>
      <section className="hero" id="top"><div className="eyebrow">OPEN-SOURCE COMMERCE STARTER</div><h1>Everyday objects,<br/><em>beautifully considered.</em></h1><p>Una tienda con catálogo persistente, sesiones seguras y reservas de inventario durante el pago.</p><a className="primary" href="#catalog">Explore collection <span>→</span></a><div className="hero-card glass" aria-hidden="true"><div className="orb">⌚</div><span>ORBIT / 05</span><strong>Designed for motion.</strong></div></section>
